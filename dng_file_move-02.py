...
save_base_dir_name = '../../dngfile/'

image_Num = 0
for fullname in fullnames[:]:
#fullname = fullnames[0]
    if fullname[-4:].lower() == ".dng":
        image_Num += 1
        try :
            ...
            if not os.path.exists('{0}{1}_{2:08d}_py.dng'.format(save_dir_name, image_datetime, image_Num)):
                os.rename(fullname, '{0}{1}_{2:08d}_py.dng'.format(save_dir_name, image_datetime, image_Num))
                photo_utilities.write_log(log_file, "{3} is moved to {0}{1}_{2:08d}_py.dng".format(save_dir_name, image_datetime, image_Num, fullname))
            else : 
                photo_utilities.write_log(log_file, "{3} is cannot moved because {0}{1}_{2:08d}_py.dng is already exist..".format(save_dir_name, image_datetime, image_Num, fullname))
